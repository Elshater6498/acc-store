...
const Delivery = () => {
  ...
  //   const message = `%0A%2A نوع الطلب %2A%3A استلام من الفرع %0A%2A اسم العميل %2A%3A ${
  //     data?.name
  //   } %0A%2A رقم العميل %2A%3A ${
  //     data?.phone
  //   } %0A---------------------------%0A${cartData
  //     .map((item) => {
  //       let addons = ''
  ...
  const onSubmit = async (data) => {
    ...
    if (total < 1) toast.warning(t('toast:minimumCost'))
      ...
      try {
        ...
        reset()
        navigate(
          `/order-details/${response.data?.data?._id}?option=${options.TAKEAWAY}`
        )
      } catch (error) {
        console.log(error)
