...
const Delivery = () => {
  ...
  const onSubmitToWhatsApp = async (data) => {
    ...
    const message = `%0A%2A نوع الطلب %2A%3A توصيل للصيدلية %0A%2A اسم العميل %2A%3A ${data.name
      } %0A%2A رقم العميل %2A%3A ${data.phone} %0A%2A عنوان العميل %2A%3A ${data.address
      } %0A---------------------------%0A${cartData
        .map((item) => {
          return `%2A الصنف %2A%3A ${item.name} %0A%2A الكمية %2A%3A ${item.quantity} %0A%2A السعر %2A%3A ${item.itemPrice / item.quantity} جنيه${item.quantity > 1 ? ` %0A%2A إجمالى الصنف %2A%3A ${item.itemPrice} جنيه` : ""}`;
        })
        .join(
          "%0A---------------------------%0A"
        )}%0A---------------------------%0A%2A إجمالى مبلغ الطلب %2A%3A ${total} جنيه %0A%2A وقت الطلب %2A%3A ${time}`;
